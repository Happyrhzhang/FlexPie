...
sensenet_t* sensenet_open(unsigned int core)
{
	...
	for(int i = 0; i < 4; i++)
		s->io->send_ddr[i] = (float*)snet_malloc(output_channel[2]*img_height[3]*img_width[3] * sizeof(float), &g_startDDR3Address2);
	{
		/***********************************L2空间复用,最后申请***********************************/



		//按照输入通道分组，1x1的权重参数要除以4
...
{
	float stride1 = 1.0f/stride;
    int inp_c = input_channel[layer_no]/3+1;
    int img_h = img_height[layer_no];
    int img_w = img_width[layer_no];
    int co, h, w;
