...
//					}
				...
				{
					case 1:
						readdata(s->io->tiling_output, s->io->tiling_buffer_read, s->io->tiling_input, 1, 112, img_tiling[step][DeviceId][1]-2, 32/8);//后面四个数字需要每层核对 input ouput
						break;
					case 3:
						readdata(s->io->tiling_input, s->io->tiling_buffer_read, s->io->tiling_output, 1, 112, img_tiling[step][DeviceId][1]-2, 64/8);//后面四个数字需要每层核对
						break;
					case 5:
						readdata(s->io->tiling_output, s->io->tiling_buffer_read, s->io->tiling_input, 1, 56, img_tiling[step][DeviceId][1]-2, 128/8);//后面四个数字需要每层核对
						break;
					case 7:
						readdata(s->io->tiling_input, s->io->tiling_buffer_read, s->io->tiling_output, 1, 56, img_tiling[step][DeviceId][1]-2, 128/8);//后面四个数字需要每层核对
						break;
					case 9:
						readdata(s->io->tiling_output, s->io->tiling_buffer_read, s->io->tiling_input, 1, 28, img_tiling[step][DeviceId][1]-2, 256/8);//后面四个数字需要每层核对
						break;
					case 11:
						readdata(s->io->tiling_input, s->io->tiling_buffer_read, s->io->tiling_output, 1, 28, img_tiling[step][DeviceId][1]-2, 256/8);//后面四个数字需要每层核对
						break;
					case 12:
						for(int i = 0; i < 3; i++)
						{
...
//				};
				...
				if((step <= 10 && step % 2 == 0))
				{
					...
					{
						if(i == 0)
							SrioSendData(DeviceId, (DeviceId + 1)%4, (uint32_t)s->io->tiling_input, (uint32_t)s->io->tobig_ddr_mem[DeviceId],  output_channel[11]*5*img_width[12]*4);
						else
							SrioSendData(DeviceId, (DeviceId + 1)%4, (uint32_t)s->io->tobig_ddr_mem[((DeviceId + 4 - i) % 4)], (uint32_t)s->io->tobig_ddr_mem[((DeviceId + 4 - i) % 4)],  output_channel[11]*5*img_width[12]*4);
...
//ps
				...
				if(step <= 10 && step % 2 == 0)
				{
					...
					if(DeviceId == 0)
					{
						SrioSendData(0, 1, (uint32_t)s->io->tiling_input, (uint32_t)s->io->tobig_ddr_mem[0],  output_channel[11]*5*img_width[12]*4);
//						SrioSendData(0, 2, (uint32_t)s->io->tiling_input, (uint32_t)s->io->tobig_ddr_mem[0],  output_channel[11]*5*img_width[12]*4);
						SrioSendData(0, 3, (uint32_t)s->io->tiling_input, (uint32_t)s->io->tobig_ddr_mem[0],  output_channel[11]*5*img_width[12]*4);
					}else if(DeviceId == 1||DeviceId ==3)
					{
						SrioSendData(DeviceId, 0, (uint32_t)s->io->tiling_input, (uint32_t)s->io->tobig_ddr_mem[DeviceId],  output_channel[11]*5*img_width[12]*4);
					}
...
//						SrioSendData(0, 3, (uint32_t)s->io->tobig_ddr_mem[2], (uint32_t)s->io->tobig_ddr_mem[2],  output_channel[11]*img_height[12]*img_width[12]);
				...
				if((step <= 10 && step % 2 == 0))
				{
					...
					for(int i = 0;i < 3;i+=2)
					{
						SrioSendData(DeviceId, (DeviceId + i + 1)%4, (uint32_t)s->io->tiling_input, (uint32_t)s->io->tobig_ddr_mem[DeviceId],  output_channel[11]*5*img_width[12]*4);
				...
				}else if(step >= 12 && step <= 25 && step % 2 == 1)
				{
					...
					for(int i = 0;i < 3;i+=2)
					{
						...
						{
						case 13:
							SrioSendData(DeviceId, (DeviceId + i + 1)%4,  (uint32_t)s->io->output_img14, (uint32_t)s->io->output_img14, 170*img_height[13]*img_width[13]*4);
							break;
						case 15:
							SrioSendData(DeviceId, (DeviceId + i + 1)%4,  (uint32_t)s->io->output_img16, (uint32_t)s->io->output_img16, 170*img_height[13]*img_width[13]*4);
							break;
						case 17:
							SrioSendData(DeviceId, (DeviceId + i + 1)%4,  (uint32_t)s->io->output_img18, (uint32_t)s->io->output_img18, 170*img_height[13]*img_width[13]*4);
							break;
						case 19:
							SrioSendData(DeviceId, (DeviceId + i + 1)%4,  (uint32_t)s->io->output_img20, (uint32_t)s->io->output_img20, 170*img_height[13]*img_width[13]*4);
							break;
						case 21:
							SrioSendData(DeviceId, (DeviceId + i + 1)%4,  (uint32_t)s->io->output_img22, (uint32_t)s->io->output_img22, 170*img_height[13]*img_width[13]*4);
							break;
						case 23:
							SrioSendData(DeviceId, (DeviceId + i + 1)%4,  (uint32_t)s->io->output_img24, (uint32_t)s->io->output_img24, (171*img_height[15]*img_width[15]+5)*4);
							break;
						case 25:
							SrioSendData(DeviceId, (DeviceId + i + 1)%4,  (uint32_t)s->io->output_img26, (uint32_t)s->io->output_img26, (342*img_height[17]*img_width[17]+2)*4);
						default:
								break;
						}
						while(1){
