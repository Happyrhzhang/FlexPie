...
int32_t RecvLocalPackages(uint32_t DoorBellInfo)
{
	int dix;
	int PackNum = 0;
	DoorBellInfo &= 0x0FFF;
	for(dix = 0; dix < 12; dix++)
	{
		if(DoorBellInfo & (0x01<<dix)){
			PackNum+=1;
		}
	}
...
int32_t RecvRemotePackages(uint32_t DoorBellInfo)
{
	int dix;
	int PackNum = 0;
	DoorBellInfo >>= 12;
...
interrupt void SRIO_Doorbell_ISR()
{
	...
	SRIOInst.RecvLocalPackNum += RecvLocalPackages(doorbell);

	RecvNum += RecvLocalPackages(doorbell);

	//WritebackCache((void *)&RecvNum, 4);

//	if(*(uint32_t*)SRIOForwardDataAddr != 0 && (RecvRemotePackages(doorbell) > 0)){
//		//收到转发包
//		SrioSendData(	DeviceId,
//						SRIOInst.DevicesID[1],
//						SRIOForwardDataAddr+8,
//						*(uint32_t*)SRIOForwardDataAddr,
//						*(uint32_t*)(SRIOForwardDataAddr+4)
//				);
//
//		*(uint32_t*)SRIOForwardDataAddr = 0;
//		WritebackCache((void *)SRIOForwardDataAddr, 4);
//		//printf("1");
//	}
//	if(*(uint32_t*)(SRIOForwardDataAddr + 0x00100000) != 0 && (RecvRemotePackages(doorbell) > 0)){
//		//收到转发包
//		SrioSendData(	DeviceId,
//						SRIOInst.DevicesID[1],
//						(SRIOForwardDataAddr + 0x00100000)+8,
//						*(uint32_t*)(SRIOForwardDataAddr + 0x00100000),
//						*(uint32_t*)((SRIOForwardDataAddr + 0x00100000)+4)
//				);
//
//		*(uint32_t*)(SRIOForwardDataAddr + 0x00100000) = 0;
//		WritebackCache((void *)(SRIOForwardDataAddr + 0x00100000), 4);
//		//printf("2");
//	}
//	if(*(uint32_t*)(SRIOForwardDataAddr + 0x00200000) != 0 && (RecvRemotePackages(doorbell) > 0)){
//		//收到转发包
//		SrioSendData(	DeviceId,
//						SRIOInst.DevicesID[1],
//						(SRIOForwardDataAddr + 0x00200000)+8,
//						*(uint32_t*)(SRIOForwardDataAddr + 0x00200000),
//						*(uint32_t*)((SRIOForwardDataAddr + 0x00200000)+4)
//				);
//
//		*(uint32_t*)(SRIOForwardDataAddr + 0x00200000) = 0;
//		WritebackCache((void *)(SRIOForwardDataAddr + 0x00200000), 4);
//		//printf("forward packs = %d\n",RecvRemotePackages(doorbell));
//	}
